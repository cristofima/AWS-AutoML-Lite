...
These instructions guide GitHub Copilot in reviewing Next.js 16+ code to ensure adherence to clean code principles, SOLID design patterns, TypeScript best practices, and Next.js-specific conventions.

---

## Version Context

| Dependency | Version | Review Focus |
|------------|---------|-------|
| Next.js | 16.0.0+ | Async params, Server Actions, caching APIs |
| React | 19.0.0+ | use() hook, concurrent features |
| TypeScript | 5.0.0+ | Strict mode, no `any` types |
| Node.js | 20.0.0+ | ES2022+ features |
...
- [ ] **[Next.js 16]** Not awaiting `params` in dynamic routes
- [ ] **[Next.js 16]** Using deprecated caching APIs

**Examples of issues to flag:**

```typescript
// 🚨 CRITICAL - Not awaiting params (Next.js 16 breaking change)
export default async function JobPage({
  params,
}: {
  params: { jobId: string }; // Wrong! Must be Promise
}) {
  const job = await getJob(params.jobId); // Will fail in Next.js 16
}

// ✅ SUGGESTION: Await params (Next.js 16 pattern)
export default async function JobPage({
  params,
}: {
  params: Promise<{ jobId: string }>; // Correct type
}) {
...
// ✅ SUGGESTION: Use SWR with conditional polling
"use client";
...
- [ ] **Async params** - `params` is Promise, must await
...
- [ ] Optimized images
- [ ] SWR conditional polling (refreshInterval with function)
